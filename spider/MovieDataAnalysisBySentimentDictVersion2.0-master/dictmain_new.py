#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2017/9/21 22:21
# @Author  : Daisy
# @Site    : 
# @File    : dictmain_new.py
...
column_comment_num = conf.getint('excel_settings',
                                 'column_comment_num')  # 读取conf中excel_settings段落中column_comment_num变量值（整型）赋给column_comment_num
column_date_num = conf.getint('excel_settings',
                              'column_date_num')  # 读取conf中excel_settings段落中column_date_num变量值（整型）赋给column_comment_num
...
old_input_file = (conf.get('file_settings', 'old_input_file'))  # 读取还未清洗的excel
input_file = (conf.get('file_settings', 'input_file'))  # 读取conf中file_settings段落中input_file变量值（文本字节流）用GBK编码并赋给input_file
output_file = (conf.get('file_settings', 'output_file'))  # 读取conf中file_settings段落中output_file变量值（文本字节流）用GBK编码并赋给output_file
...
if 1:
    # 打开表格#
    table_data = xlrd.open_workbook(input_file, encoding_override='utf-8')
    # 获取一个工作表
    table = table_data.sheet_by_index(sheet_num)
    # 获取整列数据#
    col_comment = table.col_values(column_comment_num)
    ...
    for i in range(nrows):  # 遍历点评这一列，每行点评内容放到col_comment[i]
        ...
        comment = Comment.Comment(col_comment[i])
        comment.calScore()
        print('\nComment', row, ':')
