# -*- coding: utf-8 -*-
from comment import wordDivision
from comment.wordDivision import init__1
import codecs
import os
import re
...
posdict = get_data(
    path + '/comment/sentiment dictionary/positive and negative dictionary/posdict.txt')  # 肯定极性，如：爱，安，好，帮，多
negdict = get_data(
    path + '/comment/sentiment dictionary/positive and negative dictionary/negdict.txt')  # 否定极性，如：矮，悲，笨，差，少
#导入极性词典，不同的维度应该有不同的词典，这里暂时只用同一个
posMVdict = get_data(path + '/comment/sentiment dictionary/positive and negative dictionary/posdict.txt')  # 肯定极性
negMVdict = get_data(path + '/comment/sentiment dictionary/positive and negative dictionary/negdict.txt')  # 否定极性
posdaoyandict = get_data(path + '/comment/sentiment dictionary/positive and negative dictionary/posdict.txt')  # 肯定极性
negprodict = get_data(path + '/comment/sentiment dictionary/positive and negative dictionary/negdict.txt')  # 否定极性
posstardict = get_data(path + '/comment/sentiment dictionary/positive and negative dictionary/posdict.txt')  # 肯定极性
negstardict = get_data(path + '/comment/sentiment dictionary/positive and negative dictionary/negdict.txt')  # 否定极性

disstatusdict=get_data(path + '/comment/sentiment dictionary/disturb dictionary/disturb status.txt')


# 导入 adverbs of degree dictionary,程度副词
mostdict = get_data(
    path + '/comment/sentiment dictionary/adverbs of degree dictionary/most.txt')  # 最高级，如：极，极尽，极端，充分，绝顶，，正面因子2.5，反转因子-0.5
verydict = get_data(
    path + '/comment/sentiment dictionary/adverbs of degree dictionary/very.txt')  # 很高级，如：over，过度，过,很，老，不为过，正面因子2.0，反转因子-0.2
moredict = get_data(
    path + '/comment/sentiment dictionary/adverbs of degree dictionary/more.txt')  # 较高级，如：大不了，更，越加，越，但是，足足，正面因子1.5
ishdict = get_data(
    path + '/comment/sentiment dictionary/adverbs of degree dictionary/ish.txt')  # 次高级：如：一点儿，一些，比较，怪，好生，正面因子0.5
insufficientdict = get_data(
    path + '/comment/sentiment dictionary/adverbs of degree dictionary/insufficiently.txt')  # 略高级：如：半点儿，微，略，不怎么，正面因子0.3
ordinarydict = get_data(
    path + '/comment/sentiment dictionary/adverbs of degree dictionary/ordinary.txt')  # 普通级：只有一个词：一般，正面因子0.2
advicedict = get_data(
    path + '/comment/sentiment dictionary/adverbs of degree dictionary/advice.txt')  # 建议级：有四个：希望，建议，可以，应该，正面因子-0.2
inversedict = get_data(
    path + '/comment/sentiment dictionary/adverbs of degree dictionary/inverse.txt')  # 否定级，如：不，没，无，非，不然，正面因子-1.0

# 导入 dynamic dictionary，一些常要结合语境判断的特殊词
dynamic_sentiment = get_data(
    path + '/comment/sentiment dictionary/dynamic dictionary/dynamic sentiment.txt')  # 结合语境的情感极性词，有：大，小，高，低，多，少，紧，浓，淡
dynamic_inverse = get_data(
    path + '/comment/sentiment dictionary/dynamic dictionary/dynamic inverse.txt')  # 结合特殊极性词后与常规极性相反的词，有：价格，人，和dynamic_sentiment的词相结合时，极性逆转
dynamic_remove = get_data(
    path + '/comment/sentiment dictionary/dynamic dictionary/dynamic remove.txt')  # 结合特殊极性词后无意义的词，有：山，爬山，时间（我觉得这个地方最后的时间应该是爬山时间之类的，而不能仅仅是时间）
dynamic_property = get_data(
    path + '/comment/sentiment dictionary/dynamic dictionary/dynamic property.txt')  # 同时是副词也是极性词的词，如：老，好，没多大，贼，没有等
...
def divContent(content,paraposdict,paranegdict,classification):
    # seg_content = list(wordDivision._I.cut(content))
    seg_content = list(init__1.cut(content))
    n = len(seg_content)
    i = 0
